...
//
// Purpose: Plays one of many aud files with possible randomized parameters
//
//=============================================================================
...
namespace Valve.VR.InteractionSystem
{
	...
	{
		[Tooltip( "List of aud clips to play." )]
		...
		[Tooltip( "After the aud clip finishes playing, disable the game object the sound is on." )]
		...
		void Awake()
		{
			...
			// aud source play on awake is true, just play the PlaySound immediately
		...
		{
			// get aud source properties, and do any special randomizations
			SetAudioSource();

			// if the aud source has forgotten to be set to looping, set it to looping
			if ( !audioSource.loop )
				audioSource.loop = true;

			// play the clip in the aud source, all the meanwhile updating it's location
